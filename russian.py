...
def replace_hyphenated_words(text):
    """
    Заменяет дефисы на пробелы в словах, кроме исключений.

    Args:
        text: Входной русский текст.

    Returns:
        Текст, в котором дефисы в составных словах заменены на пробелы.
    """
    # Список суффиксов и префиксов, указывающих на то, что дефис НЕ нужно заменять
    # (частицы, наречия и т.д.)
    # Можно расширить при необходимости
    exception_suffixes = ('-то', '-либо', '-нибудь', '-ка', '-таки')
    exception_prefixes = (
                            'кое-', 'по-', 'серо-', 
                            'бело-', 'черно-', 'светло-',
                            'темно-', 'ярко-'
                         )
    # Паттерн для поиска слов с дефисом.
    # \b - граница слова
    # [а-яА-ЯёЁ]+ - одна или более русских букв
    # (?:-[а-яА-ЯёЁ]+)+ - один или более сегментов "-буквы" (незахватывающая группа)
    pattern = re.compile(r'\b[а-яА-ЯёЁ]+(?:-[а-яА-ЯёЁ]+)+\b', re.IGNORECASE)

    def replacer(match):
        word = match.group(0)
        word_lower = word.lower()

        # Проверяем, не является ли слово исключением (по префиксу или суффиксу)
        is_exception = False
        if word_lower.startswith(exception_prefixes):
             # Дополнительная проверка для "по-", чтобы не заменять "по-новому" и т.п.,
             # но и не трогать "по-английски". Пока оставляем все "по-" как исключения.
             # Можно добавить более сложную логику, если нужно различать.
            is_exception = True
        elif word_lower.endswith(exception_suffixes):
            is_exception = True
        # Можно добавить проверку на полные слова-исключения, если нужно:
        elif word_lower in {'во-первых', 'во-вторых'}:
            is_exception = True

        if is_exception:
            return word # Возвращаем слово без изменений
        else:
            # Заменяем все дефисы на пробелы в найденном слове
            return word.replace('-', ' ')
...
def normalize_russian(text, expand=False, dates=False, currency=False, phone_numbers=False, numbers=True, cyrrilize=False, no_hyphen=False):
    """
    Normalize Russian text by expanding abbreviations, normalizing dates, currency, phone numbers,
    and numbers. Optionally, convert text to Cyrillic script.
    """
    # Если в тексте в сложных словах вместо дефиса "-" встречается "–" (длинный дефис),
    # заменяем его на обычный дефис "-"
    text = re.sub(r'([а-яА-ЯёЁ])–([а-яА-ЯёЁ])', r'\1-\2', text)

    if no_hyphen:
        text = replace_hyphenated_words(text)
    if expand:  
